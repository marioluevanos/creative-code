  ...
  function css() {
    return `
		...
		* {
		...
		.compare-table-row.is-header .cell-row {
			border: none;
		...
		@media (min-width: 769px) {
			.compare-table-row.is-header .cell.perk {
				padding-left: 0;
			}
		}
		.compare-table-row.is-header .cell {
			-ms-flex-align: end;
		...
		.compare-table-row.is-header .text-xlarge {
			margin: 0;
		}
		.compare-table-row.is-header .text-large {
			font-weight: 700;
			margin: 0;
		}
		...
		.compare-table-row .cell.perk .icon {
			margin: 0 8px 0 0;
			color: #52657A;
			display: -ms-flexbox;
			display: flex;
		}
		.compare-table-row .cell.perk .icon svg {
			width: 12px;
			height: 12px;
			transition: all 0.15s;
		}
		.compare-table-row .icon .minus {
			display: none;
		}
		.compare-table-row:not(.is-header) .cell.perk p {
			font-weight: 600;
			transition: opacity 0.3s cubic-bezier(0.39, 0.575, 0.565, 1);
		}
		...
		@media (hover: hover) {
			.compare-table-row.has-description .cell-row:hover p {
				opacity: 0.66;
			}
		}
		.compare-table-row .cell.perk p.text-xlarge {
			font-weight: 700;
		}
		.compare-table-row.is-active .icon .plus {
			display: none;
		}
		.compare-table-row.is-active .icon .minus {
			display: block;
		}
		...
		Highlighted Column
		-------------------------------- */
		.compare-table-row .cell.highlight {
			width: 100%;
			z-index: 1;
		}
		.compare-table-row .cell.highlight::before {
			content: "";
			position: absolute;
			top: 0;
			left: -1.5px;
			width: calc(100% - 3px);
			height: calc(100% + var(--margin) + 3px);
			background: #F0F2F5;
			border-left: 1.5px solid #D1D9E0;
			border-right: 1.5px solid #D1D9E0;
			transition: height 0.3s cubic-bezier(0.445, 0.05, 0.55, 0.95), top 0.3s cubic-bezier(0.445, 0.05, 0.55, 0.95);
			z-index: 0;
		}
		.compare-table-row.is-active .cell.highlight::before {
			top: -1.5px;
		}
		.compare-table-row.is-active:not(:last-of-type) .cell.highlight::before {
			top: -2px;
			height: calc(100% + 2px);
		}
		.compare-table-row.is-active + .compare-table-row.is-active:not(:last-of-type) .cell.highlight::before {
			top: 0;
			height: 100%;
		}
		.compare-table-row.is-active + .compare-table-row.is-active:last-of-type .highlight::before {
			top: 0;
		}
		
		.compare-table-row.is-header .highlight::before,
		.compare-table-row.is-active .highlight::before,
		.compare-table-row:only-child .highlight::before {
			top: 0;
			height: calc(100% + 0px);
		...
		.compare-table-row:last-child:not(:only-child) .cell.highlight::before {
		...
		Content (Slide Down)
		-------------------------------- */
		.compare-table-row.has-description .cell-row {
			cursor: pointer;
		}
		
		.compare-table-row .description {
			text-align: left;
			border-radius: 0 0 12px 12px;
			border: 1.5px solid #B2DED1;
			background: #E2F3EE;
			overflow: hidden;
			box-sizing: border-box;
			position: absolute;
			...
			padding: 24px;
			z-index: -1;
			transition: height 0.3s cubic-bezier(0.445, 0.05, 0.55, 0.95), padding-top 0.3s cubic-bezier(0.445, 0.05, 0.55, 0.95), padding-bottom 0.3s cubic-bezier(0.445, 0.05, 0.55, 0.95);
		}
		.compare-table-row .description.show {
			box-sizing: content-box;
			position: relative;
			visibility: visible;
			margin-top: -3px;
			height: var(--description-h, auto);
			-webkit-transform: translate3d(0, -3px, 0);
							transform: translate3d(0, -3px, 0);
		}
		.compare-table-row.is-collapsed .description {
			padding: 0 24px;
		}
		.compare-table-row .description > p {
			margin: 0;
			opacity: 0;
			-webkit-transform: translate3d(0, -8px, 0);
							transform: translate3d(0, -8px, 0);
			transition: opacity 0.15s cubic-bezier(0.445, 0.05, 0.55, 0.95) 0s, -webkit-transform 0.15s cubic-bezier(0.445, 0.05, 0.55, 0.95) 0s;
			transition: opacity 0.15s cubic-bezier(0.445, 0.05, 0.55, 0.95) 0s, transform 0.15s cubic-bezier(0.445, 0.05, 0.55, 0.95) 0s;
			transition: opacity 0.15s cubic-bezier(0.445, 0.05, 0.55, 0.95) 0s, transform 0.15s cubic-bezier(0.445, 0.05, 0.55, 0.95) 0s, -webkit-transform 0.15s cubic-bezier(0.445, 0.05, 0.55, 0.95) 0s;
		}
		.compare-table-row.is-active .description > p {
			-webkit-transform: translate3d(0, 0, 0);
							transform: translate3d(0, 0, 0);
			opacity: 1;
			transition: opacity 0.15s cubic-bezier(0.39, 0.575, 0.565, 1) 0.15s, -webkit-transform 0.15s cubic-bezier(0.39, 0.575, 0.565, 1) 0.15s;
			transition: opacity 0.15s cubic-bezier(0.39, 0.575, 0.565, 1) 0.15s, transform 0.15s cubic-bezier(0.39, 0.575, 0.565, 1) 0.15s;
			transition: opacity 0.15s cubic-bezier(0.39, 0.575, 0.565, 1) 0.15s, transform 0.15s cubic-bezier(0.39, 0.575, 0.565, 1) 0.15s, -webkit-transform 0.15s cubic-bezier(0.39, 0.575, 0.565, 1) 0.15s;
		}
		
		/*
		Active State
		-------------------------------- */
		.compare-table-row.is-active .cell-row {
			position: relative;
			z-index: 1;
		}
		.compare-table-row.is-active:only-child .highlight::before {
			top: 0;
			height: 100%;
		}
		<style>
