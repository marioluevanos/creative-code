const total = 10;
const [logo, logoMarks] = createLogo();
const svg = logo.querySelector("svg");
const logoMark = svg.querySelector('[data-logo="mark"]');
const threesixtyGroup = Array.from(
  svg.querySelectorAll('[data-logo="threesixty-group"] path')
);
const timeline = createTimeline(logoMarks);
...
function createLogo() {
  const logoMarks = Array.from({ length: total }, (_, i) => {
    const svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
    svg.setAttribute("data-i", i);
    svg.setAttribute("viewBox", "0 0 200 200");
    svg.setAttribute("class", "logo-mark");
    svg.setAttribute("fill", "var(--color-brand)");
    svg.innerHTML = `
      <path d="M100,0C44.8,0,0,44.8,0,100s44.8,100,100,100s100-44.8,100-100V0H100z M100,140.2 c-22.2,0-40.2-18-40.2-40.2s18-40.2,40.2-40.2s40.2,18,40.2,40.2S122.2,140.2,100,140.2z"/>
    `;
    return svg;
  });
  ...
  logoMarks.forEach((d) => logo.appendChild(d));
  document.body.appendChild(logo);

  appendSvg(logo);

  return [logo, logoMarks];
}

function createTimeline(logoMarks) {
...
function hideDivs() {
  logoMarks.forEach((svg) => svg.remove());
}
