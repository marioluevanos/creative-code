...
let gsap,
  ...
  _calculateChange = (velocity, duration) =>
    (duration * _checkPoint * velocity) / _checkPointRatio,
        ...
        if (!_isObject(curProp)) {
          ...
          if (tracker && tracker.isTracking(p)) {
            curProp = _isNumber(curProp)
              ? { velocity: curProp }
              : { velocity: tracker.get(p) }; //if we're tracking this property, we should use the tracking velocity and then use the numeric value that was passed in as the min and max so that it tweens exactly there.
          } else {
    ...
    if (ratio || !_reverting()) {
      while (pt) {
        pt.set(
          pt.t,
          pt.p,
          _round(pt.s + pt.c1 * ratio + pt.c2 * ratio * ratio) + pt.u,
          pt.d,
          ratio
        );
        pt = pt._next;
