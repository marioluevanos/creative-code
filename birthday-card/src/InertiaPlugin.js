(function () {
  ...
  var gsap$1,
    ...
    _calculateDuration = function _calculateDuration(start, end, velocity) {
      return Math.abs(
        ((end - start) * _checkPointRatio) / velocity / _checkPoint
      );
    },
    _reservedProps = {
      resistance: 1,
    ...
    _processLinkedProps = function _processLinkedProps(
      target,
      vars,
      getVal,
      resistance
        ...
        for (i = 0; i < linkedPropNames.length; i++) {
          ...
          if (curProp) {
            if (_isNumber(curProp.velocity)) {
              ...
              curVelocity =
                tracker && tracker.isTracking(p) ? tracker.get(p) : 0;
            }
    ...
    _calculateTweenDuration = function _calculateTweenDuration(
      ...
      linkedProps = _processLinkedProps(
        target,
        inertiaVars,
        getVal,
        resistance
        ...
        if (!_reservedProps[p]) {
          ...
          if (_isObject(curProp)) {
            ...
            curDuration = _clamp(
              ...
              Math.abs(
                curVelocity /
                  _getNumOrDefault(curProp, "resistance", resistance)
              )
            );
